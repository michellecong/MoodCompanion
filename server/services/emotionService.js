  ...
  if (sentiment) {
    ...
    if (sentiment.magnitude > 0.5) {
      if (sentiment.score < 0) {
        emotionsDetected.push({
          name: "anger",
          score: sentiment.magnitude * 0.5,
        });
        emotionsDetected.push({
          name: "anxiety",
          score: sentiment.magnitude * 0.3,
        });
      }
    }
  }
...
const detectEmotions = async (content) => {
  try {
    console.log(
      "Calling Google Cloud Natural Language API with content:",
      content
    );
...
const generateFeedback = (emotionsDetected) => {
  // Handle empty or undefined emotions array
